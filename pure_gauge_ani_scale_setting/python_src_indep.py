...
f_write = open( 'tau_Et_Es_dEt_dEs_ratio_sflow%sb%sx%s'%(vol,beta,x0) , 'w' )
...
for xf in xf_vec:
	...
	for i_file in range(first_file,n_files+first_file):
		...
		i = i_file - first_file
		f_read = open( '%s/l%sb%sx%sa/sflow%sb%sx%sxf%sa_dt%s.lat.%d'%(cur_dir,vol,beta,x0,vol,beta,x0,xf,dt,i_file) , 'r' )
...
for i_time in range(n_steps):
	Et_rec = jackknife(Et_arr[i_time,:,0],n_bins,'average')
	Et_err_rec = jackknife(Et_arr[i_time,:,0],n_bins,'error')
	Es_rec = jackknife(Es_arr[i_time,:,0],n_bins,'average')
	Es_err_rec = jackknife(Es_arr[i_time,:,0],n_bins,'error')
	dEt_rec = jackknife(dEt_arr[i_time,:,0],n_bins,'average')
	dEt_err_rec = jackknife(dEt_arr[i_time,:,0],n_bins,'error')
	dEs_rec = jackknife(dEs_arr[i_time,:,0],n_bins,'average')
	dEs_err_rec = jackknife(dEs_arr[i_time,:,0],n_bins,'error')
	ratio_rec = jackknife(ratio_arr[i_time,:,0],n_bins,'average')
	ratio_err_rec = jackknife(ratio_arr[i_time,:,0],n_bins,'error')
