...
### guard_momrcw FILE CONTAINS NUMBER OF THE NEXT LATTICE TO BE MEASURED
if [ -f "${directory}/guard_momrcw" ]
then
	i_lat=$(head -1 "${directory}/guard_momrcw")
	seed=$(tail -1 "${directory}/guard_momrcw")
else 
	i_lat=${set_i_lat}
	seed=${set_seed}
cat << EOF > "${directory}/guard_momrcw"
${i_lat}
${seed}
EOF
fi
...
do
	...
	file_name="${directory}/specmomrcw${nx}${nt}b${beta_name}x${xi_0_name}ml${mass1_name}xq${xq_0_name}${stream}.${i_lat}"
	if [ -f "${file_name}" ]
	then
		rm "${file_name}"
	fi
